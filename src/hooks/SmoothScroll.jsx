...
import { useLocation } from "react-router-dom"; // will be used only if a Router is present
...
export default function SmoothScroll({ children, ease = 0.08, className = "" }) {
    // ─── Router detection ─────────────────────────────────────────────
    // If the app is wrapped in a <Router>, we get the pathname and use it
    // to re‑sync the scroll on route changes. If not, pathname stays undefined.
    let location;
    try {
        location = useLocation();
    } catch {
        // No router present – silently ignore
        location = null;
    }
    ...
    }, [ease, pathname]); // pathname is only defined when a Router exists
